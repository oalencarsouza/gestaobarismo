        ...
        <main className="max-w-full overflow-x-hidden min-w-0 md:max-w-[1280px] mx-auto pb-12 w-full animate-in fade-in duration-500">
            <div className="relative w-full h-64 md:h-80 group">
                <div className="absolute inset-0 overflow-hidden rounded-b-[2rem]">
                    <div className="w-full h-full bg-slate-300 dark:bg-white/5 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,0.1), rgba(0,0,0,0.8)), url("https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?q=80&w=2070&auto=format&fit=crop")` }}></div>
                </div>
                {isEditing && (
                    <button className="absolute top-6 right-6 bg-black/50 hover:bg-primary text-white px-5 py-2.5 rounded-xl backdrop-blur-md flex items-center gap-2 text-sm font-bold transition-all border border-white/10 active:scale-95 shadow-xl z-20">
                ...
                <div className="absolute -bottom-16 left-6 md:left-10 flex items-end gap-4 md:gap-6 z-10">
                    <div className="relative group/avatar">
                        <div className="size-28 md:size-44 rounded-full border-4 border-[#120f0e] overflow-hidden bg-[#1a1614] shadow-2xl relative flex items-center justify-center">
                            <img className="w-full h-full object-cover" src="/logo.svg" alt="Profile" />
                            {isEditing && (
                        ...
                        {isEditing && (
                            <button className="absolute bottom-1 md:bottom-2 right-1 md:right-2 size-8 md:size-11 bg-primary text-white rounded-full flex items-center justify-center shadow-lg hover:bg-orange-600 hover:scale-110 active:scale-90 transition-all border-2 border-[#120f0e] z-20">
                                <Camera size={16} />
                            </button>
                        )}
                    </div>
                    ...
                    <div className="space-y-6 flex-1">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                defaultValue=""
                                className={`w-full bg-white/5 border rounded-xl px-4 py-3 text-white placeholder-slate-600 transition-all outline-none ${isEditing ? 'border-primary/50 focus:ring-1 focus:ring-primary/20' : 'border-white/10'}`}
                            />
                        </div>
                    </div>
                </section>
